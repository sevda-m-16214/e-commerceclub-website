...
import React, { createContext, useState, useContext, useEffect } from 'react';
// import axiosInstance from '../utils/axiosInstance'; // Not needed anymore!
...
export const AuthProvider = ({ children }) => {
    ...
    // ❌ REMOVE the helper function setAxiosAuthHeader (The Interceptor handles this)
    // const setAxiosAuthHeader = (jwtToken) => { ... };
    ...
    useEffect(() => {
        ...
        if (storedToken && storedUser) {
            try {
                ...
                setToken(storedToken);
                setUser(userData);
                // ❌ REMOVED: setAxiosAuthHeader(storedToken); 
            } catch (error) {
                console.error("Error parsing user data from localStorage:", error);
                localStorage.removeItem('jwt_token');
                localStorage.removeItem('user');
    ...
    const login = (userData, jwtToken) => {
        ...
        // ❌ REMOVED: setAxiosAuthHeader(jwtToken); 
    };

    // 1.5 Logout Function
    const logout = () => {
        // 1. Clear storage (The interceptor will now see the token is gone and won't send the header)
        localStorage.removeItem('jwt_token');
        localStorage.removeItem('user');
        ...
        // ❌ REMOVED: delete axiosInstance.defaults.headers.common['Authorization'];
    };

    // 💥 CRITICAL ADDITION: Calculate isAdmin state 💥 (This logic remains important)
    const isAdmin = user 
        ? (user.is_admin === true || user.is_admin === 1) 
        : false;
